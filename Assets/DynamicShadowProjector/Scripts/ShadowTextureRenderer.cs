...
#pragma warning restore // CS1692, RECS0001
		...
		[SerializeField]
		private UnityEngine.Camera[] m_camerasForViewClipTest;
		...
		public UnityEngine.Camera[] camerasForViewClipTest
		{
			get { return m_camerasForViewClipTest; }
			set { m_camerasForViewClipTest = value; }
		}
		public UnityEngine.Camera projectorCamera
		{
			get
			{
		...
		bool Initialize()
		{
			...
			if (m_camera == null) {
				m_camera = gameObject.GetComponent<UnityEngine.Camera>();
				if (m_camera == null) {
					m_camera = gameObject.AddComponent<UnityEngine.Camera>();
				}
		...
		void Start()
		{
...
#endif
			{
				...
				if (isEmpty)
				{
					...
					{
						m_camerasForViewClipTest = new UnityEngine.Camera[1] { UnityEngine.Camera.main };
					}
				}
				else
				{
					bool hasNonNullCamera = false;
					foreach (UnityEngine.Camera cam in m_camerasForViewClipTest)
					{
						if (cam != null)
						{
					...
					if (!hasNonNullCamera)
					{
						...
						UnityEngine.Camera[] cameras = new UnityEngine.Camera[m_camerasForViewClipTest.Length + 1];
						int i = 0;
						for (; i < m_camerasForViewClipTest.Length; ++i)
						{
							cameras[i] = m_camerasForViewClipTest[i];
						}
						cameras[i] = UnityEngine.Camera.main;
		...
		void OnPreCull()
		{
...
#endif
			{
				if (m_camerasForViewClipTest != null && 0 < m_camerasForViewClipTest.Length) {
					...
					isVisible = false;
					for (int i = 0; i < m_camerasForViewClipTest.Length; ++i) {
						UnityEngine.Camera cam = m_camerasForViewClipTest[i];
